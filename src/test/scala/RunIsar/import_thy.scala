...
class ImportsThyTests extends AnyFunSuite {
  ...
  test("Create the theorem to be proved") {
    val theorem_string = "theorem mathd_numbertheory_303: \"(\\<Sum> k \\<in> {n ::nat. 2 \\<le> n \\<and> [171 = 80] (mod n) \\<and> [468 = 13] (mod n)}. k) = 111\""
    val result1: String = isa_repl.step(theorem_string)
    assert (result1.contains("proof (prove)") && result1.contains("\\<Sum> {n. 2 \\<le> n \\<and> [171 = 80] (mod n) \\<and> [468 = 13] (mod n)} = 111"))
    isa_repl.step("sorry")
  }
